...
mod tests {
    ...
    fn counters_track_alloc_free() {
        ...
        // NB: `peak >= current` is NOT asserted — `current` and `peak` are
        // separate relaxed process-global atomics, so a concurrent test's
        // `on_alloc` can transiently make a snapshot show `current > peak`.
        on_free(100);
        on_free(200);
        let after = snapshot();
